 ...
 * @author rpatriarche
 */
define([ 'angular', 'mandy-common', 'mandy-datagrid-service' ], function(angular) {

    'use strict';

    // do not use strict mode, see sonar:javascript:StrictMode
    ...
    module.controller('DatagridController', [
        ...
        'day',
        'IGNORE_COMMENT',
        function($rootScope, $scope, $location, $log, $timeout, globals, utils, imputationService,
            datagrid, activities, imputations, year, month, day, IGNORE_COMMENT) {
            ...
            /**
             * Performs save imputation details from modal view. This action enables communication
             * between modal controller and current active view controller.
             * @fires 'md.datagrid.imputation.save'
             */
            $scope.saveModalChanges = function(imputationId, activityId, newQuota, newComment) {
                $scope.$broadcast('md.datagrid.imputation.save', imputationId, activityId,
                    newQuota, newComment);
            };

            /**
             * Updates the location when user resizes its screen in a manner that switches the view
             * mode.
             * @callback
             * @event 'md.datagrid.input.mode.changed'
             * @return A deregistration function for the vent handler.
             */
            $scope.$on('md.datagrid.input.mode.changed', function(event, arg) {
                if (arg === 'day'/* || $rootScope.preferences.enableSubActivities */) {
                    // il faut ajouter le jour au path
                    $location.path("/datagrid/year/" + year + "/month/" + month + "/day/"
                        + utils.getDay(datagrid.weeks[0].days[0].date));
                } else {
                    // retirer la partie du path correspondant au jour
            ...
            $scope.showModal = function(imputationId, activityId, date, quota) {
                $scope.$broadcast('md.datagrid.modal.show', imputationId, activityId, date, quota);
            };
            ...
            $scope.findImputation = function(activityId, imputationId) {
                // je n'utilise pas un forEach
                // angular car on
                // ne peut pas
                // utiliser break ou return à
                // l'intérieur de
                // la boucle
                // see:
            ...
            /**
             * Permet de synchroniser la base de données avec la saisie effectuée. Returns an object
             * (async token) containing two properties: - promise= called resource $promise - type=
             * the event type (C=create,D=delete,U=update)
             * @public
             */
            $scope.synchronizeBackend = function(imputationId, activityId, date, newQuota,
                newComment) {
                var resource = null;
                var asyncToken = {
                    type : null,
                ...
                if (imputationId) {
                    ...
                    if (newQuota === 0) {
                        ...
                        // UPDATE CASE
                        asyncToken.type = 'U';
                        var oldImputation = $scope.findImputation(activityId, imputationId);
                        var imputation = {};
                    ...
                    // attention ce service utilise
                    // une méthode d'instance qui
                    // retourne
                    // directement la promise
                    asyncToken.promise = imputationService.createImputation(
                        +$rootScope.user.resourceId, activityId, date, newQuota, newComment);
                }
