(function () {
  ...
  app.service('PreferencesService', [
    '$rootScope',
    '$resource',
    '$log',
    'CONTEXT_ROOT',
    ...
    function ($rootScope, $resource, $log, CONTEXT_ROOT, errorService) {
      ...
      return {
        ...
        updateUserPreferences: function (preferences) {
          return PreferencesResource.update({}, preferences, function (newPreferences) {
            // notify that user preferences have changed
            $rootScope.$broadcast('md.preferences.changed',
              newPreferences);
  ...
  app.config(['$routeProvider', function ($routeProvider) {
    ...
    $routeProvider.when('/preferences', {
      templateUrl: "app/preferences/preferences.component.html",
  ...
  app.controller('PreferencesController', [
    ...
    'preferences',
    'activities',
    function ($rootScope, $scope, $log, $timeout, utils, prefService, preferences, activities) {
       ...
       */
      angular
        .element('.pref-granularity input[data-toggle="toggle"]')
       ...
       */
      angular
        .element('.pref-detailed-way input[data-toggle="toggle"]')
        .change(function (event) {
          var status = angular.element(this).prop('checked');
          $log.debug('checked: ' + status);
      ...
      function activityVisibilityChangeEventHandler(event) {
        var activityId = +angular.element(this)
          .attr('data-activity-id');
        var visible = angular.element(this)
          .prop('checked');
        if (visible) {
          // remove if exists in filter list
          $rootScope.preferences.activitiesFilter = $.grep(
            $rootScope.preferences.activitiesFilter,
            function (currentActivityId) {
              // the item will be in the result array only if the test returns true
      ...
      $timeout(function () {
        ...
        angular
          .element('input[data-toggle="toggle"]')
          .bootstrapToggle();
        // Registers change listener (must be called when dom is rendered)
        angular
          .element('.pref-activities input[data-toggle="toggle"]')
