...
define([ 'angular', 'mandy-common' ], function(angular) {
  ...
  function performDatagridHeader($q, scope, element, utils) {
    ...
    $q.all([ scope.datagrid.$promise, scope.imputationsMap.$promise ]).then(
        function() {
  ...
  function performDatagridBodyWeek(week) {
    angular.forEach(week.days, function(day) {
      ...
      if (activityImputations) {
        var imputation = this.utils.findImputation(activityImputations,
            day.date);
        if (imputation) {
      ...
      function($rootScope, utils) {
        return {
          ...
          link : function(scope, element, attrs) {
            ...
            /**
             * TODO: check performance and potentiel memory leaks using watch
             * here
             * 
             * @handler
             */
             ...
             * 
             * @param minus
             *          If true, the current value will be reduced, if null or
             *          false the value will be increased.
             * 
             * @private
             */
