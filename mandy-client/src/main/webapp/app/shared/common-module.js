(function () {
  ...
  var UtilsService = function () {
    ...
    /**
     * Retourne vrai si la date dénotée par la chaîne passée en paramètre appartient à la
     * datagrid passée également en paramètre, faux sinon.
     * @public
     */
    ...
    /**
     * Returns the imputation matching the given date if exists, undefined if not.
     * @public
     */
    ...
    /**
     * Returns the next quota value from an existing quota value. This function use the given
     * granularity value to determine the correct value. If the next value is not compliant with
     * the current granularity, the closest compliant value is used instead.
     */
  ...
  app.service('Globals', GlobalsService);
  ...
  var ErrorService = [
    ...
    function ($rootScope, $location, $log, $timeout) {
      ...
      this.errorHandler = function (httpResponse, context) {
        ...
        status = httpResponse.status === undefined ? "???" : httpResponse.status;
        if (status === 0) {
          // socket error (server is down)
          title = "Serveur inaccessible";
          details =
            "Le serveur ne répond pas, veuillez contacter l'administrateur de l'application pour qu'il redémarre le bouzin.";
        } else if (status === 401) {
          // force redirection to login form
          ...
          stack = stack
            .substring((stack.indexOf("<body>") + 6), stack.indexOf("</body>"));
  ...
  app.controller('ErrorController', ['$rootScope', '$scope', '$sce', 'missingRoute',
    function ($rootScope, $scope, $sce, missingRoute) {
