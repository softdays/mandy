(function () {
  ...
  app.controller('DatagridMonthViewController', [
    '$rootScope',
    '$scope',
    '$location',
    '$log',
    ...
    'ImputationService',
    'IGNORE_COMMENT',
    function ($rootScope, $scope, $location, $log, $timeout, $route, $q, utils,
      ...
      var updateCompletenessMarker = function (strDate) {
        ...
        if (elem) {
          ...
          var glyphiconClass = utils.getCompletenessGlyphiconClass($scope.imputationsMap,
            strDate);
      ...
      /**
       * Returns true if at least one of new data differs from its previous value, false if
       * not.
       * @private
       */
      ...
      var updateImputation = function (ngCell, newQuota, newComment) {
        ...
        var syncBackendRequired = isSyncRequired(activityId, imputationId, newQuota,
          newComment);
        if (syncBackendRequired) {
          var asyncToken = $scope.synchronizeBackend(+imputationId, +activityId, date,
            newQuota, newComment);
          ...
          case 'D':
            asyncToken.promise.then(function () {
              ...
              var index = $scope.findImputationIndex(activityId, imputationId);
          ...
          case 'U':
            asyncToken.promise.then(function (updatedImputation) {
              ...
              var i = $scope.findImputation(activityId, imputationId);
          ...
          case 'C':
            asyncToken.promise.then(function (createdImputation) {
              // synchroniser le modèle client
      ...
      $scope.$on('md.datagrid.imputation.save', function (event, imputationId, activityId,
        ...
        if (currentCellSelected) {
          updateImputation(currentCellSelected, newQuota, newComment);
        } else {
          $log.warn("No current selected cell to perform save event");
        }
      });
      ...
      $scope.datagrid.$promise.then(function () {
        $scope.formattedMonth = utils.formatMonthLabel($scope.datagrid.month);
      });
      ...
      $scope.tableRightClickHandler = function (event) {
        ...
        if (ngCell.hasClass("selected")) {
          ...
          var newQuota = utils.getNewQuota(oldQuota, $rootScope.preferences.granularity);
