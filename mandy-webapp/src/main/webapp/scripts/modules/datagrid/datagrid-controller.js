      ...
      function($rootScope, $scope, $location, $log, $timeout, globals, utils,
        ...
        $scope.$on('md.datagrid.input.mode.changed', function(event, arg) {
          if (arg === 'day') {
            ...
            // retirer la partie du path correspondant
            // au jour
            $location.path("/datagrid/year/" + year + "/month/" + month);
          }
        });
        ...
        $scope.findImputation = function(activityId, imputationId) {
          // je n'utilise pas un forEach angular car on
          // ne peut pas
          // utiliser break ou return à l'intérieur de
          // la boucle
          // see:
        ...
        $scope.synchronizeBackend = function(imputationId, activityId, date,
          ...
          if (imputationId) {
            // Imputation already exists, we have
            // to change it.
            if (newQuota === 0) {
              // DELETE CASE
              asyncToken.type = 'D';
            ...
            // attention ce service utilise une méthode d'instance qui retourne
