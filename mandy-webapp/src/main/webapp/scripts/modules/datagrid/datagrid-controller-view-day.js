...
define([ 'angular', 'angular-route', 'blockui', 'mandy-common', 'mandy-datagrid-service',
    'mandy-datagrid-controller-view-day-detailed' ], function(angular) {
    'use strict';

    var module = angular.module('mdDatagridControllerViewDay', [ 'ngRoute', 'mdCommon',
        'mdDatagridService', 'mdDatagridControllerViewDayDetailed' ]);

    module.controller('DatagridViewDayController', [
        '$rootScope',
        '$scope',
        '$location',
        '$log',
        ...
        function($rootScope, $scope, $location, $log, $timeout, $route, $q, globals, utils,
            imputationService, IGNORE_COMMENT) {
            ...
            $scope.$on('md.preferences.changed', function(event, prefs) {
                // update
                // granularity
                $scope.quotas = utils.getQuotasModel(prefs.granularity);
            });

            /**
             * @private
             */
            var isSyncRequired = function(pmItem, newQuotaValue, newComment) {
                return !pmItem.imputation || pmItem.imputation.quota !== newQuotaValue
            ...
            /**
             * Returns false if the comment button should be disabled, true if not.
             * @binding
             */
            $scope.isCommentButtonDisabled = function(item) {
                return !$scope.workingDay || item.imputation === undefined
                    || item.imputation.quota === 0;
            };

            var preparePresentationModel = function(strDate) {
                var data = [];
                angular.forEach($scope.activities, processActivity, data);
            ...
            function processActivity(activity) {
                var _data = this; // context has been set with an empty array
                var item = {};
                ...
                if (activityImputations) {
                    var imputation = utils.findImputation(activityImputations, strDate);
                    if (imputation) {
                        item.imputation = imputation;
                        item.printQuota = utils.formatQuota(imputation.quota);
                    }
                }
                _data.push(item);
            }
            ...
            var findImputationPmItem = function(imputationId) {
                for (var i = 0; i < $scope.preparedDataForDayView.length; i++) {
                    ...
                    // skip null imputation corresponding to empty imputation for an activity
                    if (item.imputation && item.imputation.imputationId === imputationId) {
            ...
            /**
             * Registers the save event handler when the user switch day/month view mode.
             * @callback
             * @event 'md.datagrid.imputation.save'
             * @return A deregistration function for this event handler.
             */
            $scope.$on('md.datagrid.imputation.save', function(event, imputationId, activityId,
                newQuota, newComment) {
                var item = findImputationPmItem(imputationId);
                if (item) {
                    $scope.updateImputation(item, newQuota, newComment);
                } else {
                    $log.warn("No presentation model " + "item found to" + " perform save event");
                }
            });
             ...
             * @binding
             */
            $scope.quotas = utils.getQuotasModel($rootScope.preferences.granularity);
            ...
            function updateCompletenessIndicator() {
                ...
                var iconName = utils.getCompletenessGlyphiconClass($scope.imputationsMap, strDate);
            ...
            /**
             * TODO: services should directly return promises (that way angular will resolve them
             * before instantiate the controller)
             */
            $q.all(
                [ $scope.datagrid.$promise, $scope.activities.$promise,
                    $scope.imputationsMap.$promise ]).then(asyncProcess);

            /**
             * 
             */
            function asyncProcess() {
                $scope.workingDay = utils.isWorkingDay(strDate, $scope.datagrid);

                updateCompletenessIndicator();

                $scope.$watch('workingDay', function(newValue) {
                    if (newValue) {
                        // déblocage de la saisie
                        $('.datagrid-body-view-day').unblock();
                        ...
                        $('.datagrid-body-view-day').block(params);

                        // désactivation du scroll (sinon pb avec le blockui qui
                        // a un z-index très
                        // élevé)
                        $('html, body').css({
                            'overflow' : 'hidden',
            ...
            $scope.displayNextDay = function() {
                $location.path("/datagrid/" + utils.buildPathForNextDay(strDate));
            };
            ...
            $scope.displayPreviousDay = function() {
                $location.path("/datagrid/" + utils.buildPathForPreviousDay(strDate));
            };
             ...
             * @action
             */
            $scope.updateImputation = function(pmItem, newQuotaValue, newComment) {
                ...
                if (isSyncRequired(pmItem, newQuotaValue, newComment)) {
                    ...
                    // scope
                    // function
                    var imputationId = pmItem.imputation ? pmItem.imputation.imputationId : null;
                    var asyncToken = $scope.synchronizeBackend(imputationId, pmItem.activity.id,
                        utils.getDate(strDate), newQuotaValue, comment);
                    asyncToken.promise.then(function(imputation) {
                        ...
                        case 'D':
                            ...
                            // from
                            // model
                            var index = $scope.findImputationIndex(pmItem.activity.id,
                                pmItem.imputation.imputationId);
                            $scope.imputationsMap[pmItem.activity.id].splice(index, 1);
                            // update
                            // pm
                        ...
                        case 'C':
                            ...
                            pmItem.printQuota = utils.formatQuota(imputation.quota);
                            break;
