(function () {
  ...
  app.directive('mdDatagridHeader', ['$q', 'Utils', function ($q, utils) {
    return {
      restrict: 'A',
      scope: false,
      replace: false,
  ...
  function performDatagridHeader($q, scope, element, utils) {
    ...
    $q.all([scope.datagrid.$promise, scope.imputationsMap.$promise])
      .then(function () {
  ...
  app.directive('mandyDatagridBody', ['$q', '$rootScope', 'Utils',
  ...
  app.directive('mdDatagridSlider', [
    '$rootScope',
    'Utils',
    function ($rootScope, utils) {
      return {
        // refer external template
        templateUrl: 'app/datagrid/datagrid-slider.html',
